...
// size, schema drift, dedup, row-count reconciliation, custom SQL, freshness) — named here so
// the Home dashboard's check count is derived, not a hardcoded number that drifts out of sync
// as mechanisms are added. Row-count reconciliation lives on the Review step
// (simulateRowCountReconciliation in dataQuality.ts) rather than the Data Quality tab itself,
// and custom SQL checks (CustomSqlChecks.tsx) are user-authored rather than built-in, but all
// are real DQ mechanisms this platform runs, so all count here too.
export const NON_COLUMN_DQ_MECHANISMS = [
  'File presence',
  'File size',
  'Schema drift',
  'Dedup / golden-record',
